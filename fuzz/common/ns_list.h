...
/** \hideinitializer \brief Check if a list contain an entry.
 *
 * \param list  `(list_t *)`  Pointer to list.
 * \param entry `(entry_t *)` Entry on list to be check.
 */
#define ns_list_contain(list, entry) \
  ns_list_contain_(&(list)->slist, NS_LIST_OFFSET_(list), NS_LIST_TYPECHECK_(list, entry))
...
NS_INLINE bool ns_list_contain_(ns_list_t *list, ns_list_offset_t link_offset, void *entry);
...
NS_LIST_FN bool ns_list_contain_(ns_list_t *list, ns_list_offset_t offset, void *entry)
{
  for (void *p = list->first_entry; p; p = NS_LIST_NEXT_(p, offset)) {
    if (p == entry) {
      return true;
